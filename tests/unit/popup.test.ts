...
describe("popup readiness surface", () => {
  ...
  it("offers the readiness region as a keyboard focus target with a visible focus outline", () => {
    loadPopupMarkup();
    loadPopupStyles();
    ...
    expect(readinessRegion?.getAttribute("tabindex")).toBe("0");

    readinessRegion?.focus();

    expect(document.activeElement).toBe(readinessRegion);

    const focusVisibleRule = Array.from(document.styleSheets)
      .flatMap((styleSheet) => Array.from(styleSheet.cssRules))
      .find(
        (rule): rule is CSSStyleRule =>
          "selectorText" in rule && rule.selectorText === "#readiness-region:focus-visible"
      );
